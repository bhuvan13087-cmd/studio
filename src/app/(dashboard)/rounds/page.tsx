          ...
          <DialogContent className="sm:max-w-[425px]">
            <form onSubmit={handleAddChit}>
              ...
              <div className="grid gap-5 py-6">
                ...
                <div className="grid grid-cols-2 gap-4"><div className="grid gap-2"><Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Start Date</Label><Input type="date" value={newChit.startDate ?? ""} onChange={e => setNewChit({...newChit, startDate: e.target.value})} required className="h-11 rounded-xl" /></div><div className="grid gap-2"><Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">End Date</Label><Input type="date" value={newChit.endDate ?? ""} onChange={e => setNewChit({...newChit, endDate: e.target.value})} required className="h-11 rounded-xl" /></div></div>
              </div>
        ...
        <DialogContent className="sm:max-w-[425px]">
          {selectedMemberForPayment && (
            <form onSubmit={handleQuickPayment}>
              ...
              <DialogFooter className="mt-4"><Button type="submit" disabled={isActionPending} className="w-full h-12 font-black uppercase tracking-[0.2em] bg-emerald-600 hover:bg-emerald-700 shadow-lg active:scale-[0.98] transition-all">{isActionPending ? <Loader2 className="mr-2 animate-spin" /> : <CheckCircle2 className="mr-2 size-5" />}Confirm Entry</Button></DialogFooter>
            </form>
          )}
        </DialogContent>
      </Dialog>

...
export default function DashboardPage() {
  ...
  // Cleanup effect to ensure UI is interactive after any modal unmounts
  useEffect(() => {
    return () => {
  ...
  const dashboardData = useMemo(() => {
    ...
    return { activeMembersCount: members?.filter(m => m.status !== 'inactive').length || 0, collectedThisMonth: collectedThisCycle, collectedToday, dailyPendingCount: dailyPendingList.length, monthlyOverdueCount: monthlyOverdueList.length, schemeSummaries, recentPaymentsList: (payments || []).filter(p => {
      if (p.status !== 'paid' && p.status !== 'success' && p.status !== undefined) return false;
      const member = members?.find(m => m.id === p.memberId);
  ...
  const getDisplayName = (name: string) => { if (!name) return ""; const clean = name.replace(/Group/gi, '').trim(); return `Group ${clean}`; };
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-10 overflow-x-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1.5">
          ...
          <p className="text-sm sm:text-base text-muted-foreground font-medium flex items-center gap-2"><ShieldCheck className="size-4 text-primary" /> Cycle-Based Operational Status.</p>
        </div>
        <Button variant="outline" className="font-bold gap-2 h-11 px-6 shadow-sm border-border/60 hover:bg-muted/50 transition-all active:scale-95" onClick={() => window.location.reload()}><RefreshCcw className="size-4 text-primary" /> Sync Status</Button>
      </div>
        ...
        <Card className="hover:shadow-xl transition-all duration-300 border-border/60 rounded-2xl bg-card shadow-sm group">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0 p-6">
            ...
            <Select value={revenueView} onValueChange={(v: any) => setRevenueView(v)}>
              <SelectTrigger className="h-8 w-fit min-w-[100px] text-[10px] font-black border-none bg-muted/50 focus:ring-0 shadow-none px-3 rounded-lg"><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="month" className="text-[10px] font-black uppercase">Active Cycle</SelectItem><SelectItem value="today" className="text-[10px] font-black uppercase">Captured Today</SelectItem></SelectContent>
            </Select>
          </CardHeader>
          <CardContent className="p-6 pt-0"><div className="text-4xl font-black tracking-tighter text-primary tabular-nums mb-1">₹{(revenueView === 'month' ? collectedThisMonth : collectedToday).toLocaleString()}</div><div className="text-[11px] text-muted-foreground font-semibold mt-2 flex items-center gap-1.5"><IndianRupee className="size-3.5 text-emerald-600" /> {revenueView === 'month' ? 'Total Cycle Revenue' : 'Real-time Daily Intake'}</div></CardContent>
        </Card>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {schemeSummaries.map((scheme, i) => (
            <Card key={i} className="group cursor-pointer hover:border-primary hover:shadow-xl transition-all border-border/60 overflow-hidden relative shadow-sm rounded-2xl" onClick={() => { setSelectedGroup(scheme); setIsGroupDetailOpen(true); }}>
              ...
              <CardHeader className="p-5 pb-2 bg-muted/10 border-b border-border/40"><div className="flex items-center justify-between"><CardTitle className="text-[11px] font-black uppercase text-muted-foreground tracking-[0.3em] truncate">{getDisplayName(scheme.name)}</CardTitle><Badge variant="outline" className="text-[8px] font-black border-none bg-primary/5 text-primary/60">{scheme.collectionType}</Badge></div></CardHeader>
              <CardContent className="p-5"><div className="flex items-end justify-between"><div><div className={cn("text-3xl font-black tabular-nums transition-colors flex items-center gap-3 mb-1", scheme.totalPendingDays > 0 ? (scheme.collectionType === 'Daily' ? "text-destructive" : "text-amber-600") : "text-emerald-600")}>{scheme.totalPendingDays}</div><p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 flex items-center gap-1.5"><Clock className="size-3" /> {scheme.collectionType === 'Daily' ? 'Arrears Days' : 'Overdue Span'}</p></div><div className="text-[10px] font-black text-muted-foreground/30 uppercase tracking-widest">{scheme.memberCount} Seats</div></div></CardContent>
            </Card>
          ))}
        </div>
      </div>
      ...
      <Card className="border-border/60 overflow-hidden shadow-sm rounded-2xl border bg-card">
        <CardHeader className="bg-muted/20 p-6 flex flex-row items-center justify-between border-b border-border/50"><div className="space-y-1"><CardTitle className="flex items-center gap-2 text-xl font-black tracking-tight uppercase"><CheckCircle2 className="size-6 text-emerald-500" />Cycle Transactions</CardTitle><CardDescription className="text-xs font-medium">Verified automated collection logs for current cycle.</CardDescription></div></CardHeader>
        <CardContent className="p-6">
               ...
               {recentPaymentsList.map((payment, i) => {
                 ...
                 return (
                   <div key={i} className="flex flex-col p-5 rounded-2xl bg-muted/20 hover:bg-muted/40 transition-all border border-transparent hover:border-border/60 group">
                      <div className="flex items-center justify-between mb-3"><Badge className="bg-emerald-100 text-emerald-700 border-none text-[9px] font-black uppercase tracking-widest">Verified</Badge><span className="text-[10px] text-muted-foreground font-bold tabular-nums">{pDateStr}</span></div>
           ...
           ) : (<div className="h-[120px] flex items-center justify-center text-muted-foreground italic text-xs font-medium border-2 border-dashed rounded-2xl">No active cycle logs captured.</div>)}
        </CardContent>
      </Card>

      <Dialog open={isGroupDetailOpen} onOpenChange={(open) => { if (!open) { setSelectedGroup(null); document.body.style.pointerEvents = 'auto'; } setIsGroupDetailOpen(open); }}><DialogContent className="sm:max-w-[650px] max-h-[85vh] flex flex-col p-0 overflow-hidden rounded-2xl">{selectedGroup && (<><DialogHeader className="p-6 border-b bg-muted/10 space-y-4"><div className="flex items-center gap-4"><div className="h-12 w-12 rounded-2xl bg-primary/10 text-primary flex items-center justify-center font-black shadow-inner"><FolderKanban className="size-6" /></div><div><DialogTitle className="text-2xl font-black uppercase tracking-tight">{getDisplayName(selectedGroup.name)} Registry</DialogTitle><DialogDescription className="text-[10px] font-black uppercase tracking-[0.2em] text-primary/70 mt-1">Mode: {selectedGroup.collectionType} Collection Board</DialogDescription></div></div></DialogHeader><div className="flex-1 overflow-y-auto"><Table><TableHeader className="bg-muted/30 sticky top-0 z-10"><TableRow className="hover:bg-transparent"><TableHead className="text-[10px] uppercase font-black tracking-[0.2em] pl-8 h-12">Participant</TableHead><TableHead className="text-[10px] uppercase font-black tracking-[0.2em] h-12">Status Indicator</TableHead><TableHead className="text-[10px] uppercase font-black tracking-[0.2em] text-center h-12">Arrears/Overdue</TableHead></TableRow></TableHeader><TableBody>{selectedGroup.members.length > 0 ? selectedGroup.members.map((m: any) => { const status = m.memberStatus; const isMonthly = (m.paymentType || selectedGroup.collectionType) === "Monthly"; return (<TableRow key={m.id} className="hover:bg-muted/10 transition-colors border-b last:border-none"><TableCell className="pl-8 py-5"><div className="flex flex-col gap-0.5"><span className="text-sm font-bold tracking-tight">{m.name}</span><span className="text-[10px] font-bold text-muted-foreground tracking-widest tabular-nums uppercase">{m.paymentType || selectedGroup?.collectionType}</span></div></TableCell><TableCell><Badge variant={status === 'paid' ? 'default' : 'secondary'} className={cn("text-[9px] font-black uppercase tracking-widest px-3 py-1 border-none shadow-sm", status === 'paid' ? "bg-emerald-500 hover:bg-emerald-600" : (status === 'waiting' ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"))}>{status === 'paid' ? 'SUCCESS' : (status === 'waiting' ? 'DUE' : (isMonthly ? 'OVERDUE' : 'PENDING'))}</Badge></TableCell><TableCell className="text-center pr-8"><button className={cn("inline-flex items-center justify-center px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-90 shadow-sm border", m.calculatedPendingDays > 0 ? "bg-destructive/10 text-destructive border-destructive/20" : "bg-emerald-50 text-emerald-700 border-emerald-200")} onClick={() => { setSelectedMemberDebt(m); setIsMemberArrearsOpen(true); }}>{m.calculatedPendingDays} {isMonthly ? 'Days Overdue' : 'Days Arrears'}</button></TableCell></TableRow>); }) : (<TableRow><TableCell colSpan={3} className="h-48 text-center text-muted-foreground italic text-xs font-bold uppercase tracking-widest">No participants available.</TableCell></TableRow>)}</TableBody></Table></div><DialogFooter className="p-4 border-t bg-muted/5"><Button onClick={() => setIsGroupDetailOpen(false)} className="w-full font-black uppercase tracking-[0.2em] h-12 rounded-xl">Close Board</Button></DialogFooter></>)}</DialogContent></Dialog>

      <Dialog open={isMemberArrearsOpen} onOpenChange={(open) => { if (!open) { setSelectedMemberDebt(null); document.body.style.pointerEvents = 'auto'; } setIsMemberArrearsOpen(open); }}><DialogContent className="sm:max-w-[420px] rounded-2xl p-0 overflow-hidden border-none shadow-2xl">{selectedMemberDebt && (<><DialogHeader className="p-6 bg-gradient-to-br from-muted/50 to-background border-b"><div className="flex items-center gap-4"><div className="h-14 w-14 rounded-2xl bg-destructive/10 text-destructive flex items-center justify-center font-black shadow-inner"><User className="size-7" /></div><div className="space-y-0.5"><DialogTitle className="text-xl font-black uppercase tracking-tight">Financial Audit</DialogTitle><DialogDescription className="text-[10px] font-black tracking-[0.2em] text-muted-foreground uppercase">{selectedMemberDebt.name}</DialogDescription></div></div></DialogHeader><div className="p-6 space-y-6 bg-background"><div className="flex flex-col items-center justify-center p-8 bg-destructive/5 rounded-3xl border border-dashed border-destructive/20 text-center relative overflow-hidden group"><p className="text-[10px] font-black uppercase tracking-[0.3em] text-destructive/60 mb-3 relative z-10">Unpaid Cycle Debt</p><div className="text-5xl font-black text-destructive tabular-nums tracking-tighter relative z-10 mb-4">₹{(selectedMemberDebt.calculatedPendingAmount || 0).toLocaleString()}</div><Badge className="bg-destructive text-destructive-foreground px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg shadow-destructive/20 relative z-10">⏳ {selectedMemberDebt.calculatedPendingDays || 0} {selectedMemberDebt.memberStatus === 'pending' ? (selectedMemberDebt.paymentType === 'Daily' ? 'Missed' : 'Overdue') : 'Pending'}</Badge></div><div className="p-5 rounded-2xl bg-primary/5 border border-primary/10 flex items-start gap-4"><Info className="size-5 text-primary shrink-0 mt-0.5" /><p className="text-[11px] text-muted-foreground leading-relaxed italic font-medium">Calculated based on {selectedMemberDebt.paymentType} mode strict cycle isolation rules.</p></div></div><DialogFooter className="p-6 pt-0 bg-background"><Button onClick={() => setIsMemberArrearsOpen(false)} className="w-full font-black uppercase tracking-[0.2em] h-12 rounded-xl active:scale-95 shadow-lg">Close Audit</Button></DialogFooter></>)}</DialogContent></Dialog>
    </div>
  )
}
